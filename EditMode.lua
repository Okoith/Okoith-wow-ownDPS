...
-- Einbindung in den WoW-Bearbeitungsmodus über LibEditMode (SPEC Abschnitt 5, Position).
-- API laut Wiki von p3lim-wow/LibEditMode: AddFrame, AddFrameSettings,
-- AddFrameSettingsButtons, RegisterCallback, GetActiveLayoutName.
--
-- Positionen liegen pro Layout im Profil: profile.layouts[layoutName] = { point, relPoint, x, y }.
-- profile.position ist die bisherige Position (bis 0.3.x) und dient als Startwert für
-- Layouts ohne eigenen Eintrag (Migration).
...
local DEFAULT_POSITION = { point = "CENTER", x = 0, y = -180 }
...
-- Setzt die Position im aktiven Layout auf den Standard (Menü und /owndps reset).
...
-- Auswahl-Frames (lib.frameSelections[frame], nicht dokumentiert). Bei Sperre wird
-- dieses Skript entfernt, sonst wiederhergestellt. Pfeiltasten und "Position
-- zurücksetzen" im Dialog fängt onPositionChanged ab.
...
local function onCreate(layoutName, _, sourceLayoutName)
  ...
  p.layouts[layoutName] = copyPosition(source or DEFAULT_POSITION)
end
...
function EditMode:Init()
  ...
  local frame = ns.Display:GetFrame()
  local ok, err = pcall(function()
    LEM:AddFrame(frame, onPositionChanged, DEFAULT_POSITION, "OwnDPS")
...
-- Nach Profilwechsel oder Änderung der Sperre
